...
- **Sandbox Chromium**: desde la **1.1.1**, los switches (`disable-setuid-sandbox`, clase X11) se aplican en `bootstrap.ts` **antes** de cargar el resto del proceso principal; en **1.1.0** el switch en `main.ts` podía aplicarse demasiado tarde (después de los `import`), de modo que el arranque no reflejaba el cambio.
