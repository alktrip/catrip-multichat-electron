/**
 * Equivalente a `UserIcon` en catrip-multichat (PyQt): SVG de burbuja con gradientes;
 * `getNewIconSvg()` replica `UserIcon.get_new_icon_svg()`.
 */

import crypto from "node:crypto";

/** Plantilla como `UserIcon.ICON_DEFAULT` (placeholder `{}` para badge en PyQt). */
const ICON_DEFAULT_TEMPLATE = `<?xml version="1.0" encoding="utf-8"?>
...
/** Igual que `UserIcon.get_new_icon_svg()` en PyQt; lista para mostrar (sin placeholder `{}`). */
export function getNewIconSvg(): string {
  const c1 = randomRgb();
  const c2 = randomRgb();
  let svg = ICON_DEFAULT_TEMPLATE.replace(/#209232/g, c1);
  svg = svg.replace(/#34c640/g, c2);
