...
**Versión actual:** **1.3.2** — no leídos en tray/dock, invitaciones a grupo, badge en dock, CI/AppImage y cierre estable desde la bandeja del sistema.
...
Artefactos en **`release/`** (versión actual en `package.json`, **1.3.2**):

```
release/
├── catrip-connect_1.3.2_amd64.deb
└── catrip-connect_1.3.2_x86_64.AppImage
```
...
```bash
sudo apt install ./release/catrip-connect_1.3.2_amd64.deb
```
...
```bash
chmod +x release/catrip-connect_1.3.2_x86_64.AppImage
./release/catrip-connect_1.3.2_x86_64.AppImage
```

Para integrar menú e iconos en el escritorio del usuario (opcional pero recomendable si no usas AppImageLauncher):

```bash
_scripts/install-appimage.sh release/catrip-connect_1.3.2_x86_64.AppImage
# revertir:
