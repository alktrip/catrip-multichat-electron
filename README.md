...
**Versión actual:** **1.4.0** — notificaciones multi-cuenta, integración Wayland (acciones del lanzador, foco al clic), actualizaciones beta/estable con verificación `.deb`, rendimiento GPU y UI refinada.
...
Artefactos en **`release/`** (versión actual en `package.json`, **1.4.0**):
...
```bash
sudo apt install ./release/catrip-connect_1.4.0_amd64.deb
```
...
```bash
chmod +x release/catrip-connect_1.4.0_x86_64.AppImage
./release/catrip-connect_1.4.0_x86_64.AppImage
```

Para integrar menú e iconos en el escritorio del usuario (opcional pero recomendable si no usas AppImageLauncher):

```bash
_scripts/install-appimage.sh release/catrip-connect_1.4.0_x86_64.AppImage
# revertir:
