...
**Versión actual:** **1.5.1** — diálogo de actualización con scroll y notas completas; incluye 1.5.0 (centro de actividad, acciones pendientes, notificaciones con preview, contador de no leídos fiel, ventana al restaurar desde bandeja, preview en navegador).
...
Artefactos en **`release/`** (versión actual en `package.json`, **1.5.1**):

```
release/
├── catrip-connect_1.5.1_amd64.deb
└── catrip-connect_1.5.1_x86_64.AppImage
```
...
```bash
sudo apt install ./release/catrip-connect_1.5.1_amd64.deb
```
...
```bash
chmod +x release/catrip-connect_1.5.1_x86_64.AppImage
./release/catrip-connect_1.5.1_x86_64.AppImage
```

Para integrar menú e iconos en el escritorio del usuario (opcional pero recomendable si no usas AppImageLauncher):

```bash
_scripts/install-appimage.sh release/catrip-connect_1.5.1_x86_64.AppImage
# revertir:
