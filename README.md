...
**Versión actual:** **1.5.0** — centro de actividad, bandeja de acciones pendientes, notificaciones con preview, contador de no leídos más fiel, ventana que recuerda tamaño/posición al volver desde la bandeja, y modo preview en navegador para desarrollo (`npm run dev`).
...
Artefactos en **`release/`** (versión actual en `package.json`, **1.5.0**):

```
release/
├── catrip-connect_1.5.0_amd64.deb
└── catrip-connect_1.5.0_x86_64.AppImage
```
...
```bash
sudo apt install ./release/catrip-connect_1.5.0_amd64.deb
```
...
```bash
chmod +x release/catrip-connect_1.5.0_x86_64.AppImage
./release/catrip-connect_1.5.0_x86_64.AppImage
```

Para integrar menú e iconos en el escritorio del usuario (opcional pero recomendable si no usas AppImageLauncher):

```bash
_scripts/install-appimage.sh release/catrip-connect_1.5.0_x86_64.AppImage
# revertir:
